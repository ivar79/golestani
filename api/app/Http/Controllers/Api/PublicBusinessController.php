...
class PublicBusinessController extends Controller {
 public function show(string $slug): JsonResponse { return response()->json(Business::where('slug',$slug)->where('status','approved')->firstOrFail()->only(['name','slug','category','services','description','phone','email','address','city','neighborhood','latitude','longitude','social_links','badges','logo','cover_image','created_at','updated_at'])); }
