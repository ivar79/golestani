...
class DesignerController extends Controller {
 public function profile(Request $request): JsonResponse { $designer=Designer::with('portfolios')->firstOrCreate(['user_id'=>$request->user()->id],['display_name'=>'طراح اینکارت','slug'=>'designer-'.Str::lower(Str::random(8)),'status'=>'pending']); if($designer->wasRecentlyCreated && $request->user()->hasRole('user') && !$request->user()->hasRole('designer')) { $request->user()->assignRole('designer'); } return response()->json($designer->load('portfolios')); }
 public function update(DesignerRequest $request): JsonResponse { $designer=Designer::firstOrCreate(['user_id'=>$request->user()->id],['display_name'=>$request->validated('display_name'),'slug'=>'designer-'.Str::lower(Str::random(8)),'status'=>'pending']); if($designer->wasRecentlyCreated && $request->user()->hasRole('user') && !$request->user()->hasRole('designer')) { $request->user()->assignRole('designer'); } $designer->update($request->validated()+['status'=>'pending','moderation_note'=>null]); return response()->json($designer->fresh()); }
