...
- `npm run build` (web): **PASS** (all routes compiled)
...
- `backup/ui-stitch-migration/home-components/` — snapshot of all migrated homepage components and `homepage.ts` hook
- `backup/ui-stitch-migration/api-auth/` — snapshot of recovery-code migration, AuthController, and artisan command
- `backup/phase-current/` — full copy of `docs/` at migration time

## 9. Emergency Admin Recovery — how to use

1. While the owner still has access, run on the server:
   `php artisan admin:recovery-code {phone}` (e.g. `php artisan admin:recovery-code 09121234567`)
2. Store the printed code safely — it is shown only once.
3. If SMS ever goes down, log in via `POST /api/auth/verify-recovery` with `{"phone": "...", "code": "..."}`.
4. The code is single-use — it is cleared on successful login. Run the artisan command again to issue a new one.

Codes are stored hashed in `users.recovery_code`, hidden from API responses, and the route is throttled (5 attempts / 15 min).
