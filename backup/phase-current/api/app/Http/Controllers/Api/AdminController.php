...
class AdminController extends Controller
{
    ...
    {
        $settings = SiteSetting::whereIn('key', ['homepage.hero.title','homepage.hero.subtitle','homepage.hero.buttons','homepage.hero.badges','homepage.hero.button_primary','homepage.hero.button_secondary','homepage.features','homepage.cta','seo.homepage'])->pluck('value','key');
        return response()->json($settings);
    }

    public function settings(): JsonResponse { return response()->json(SiteSetting::orderBy('key')->get()); }
    public function saveSetting(Request $request, string $key): JsonResponse { abort_unless(preg_match('/^[A-Za-z0-9._-]+$/', $key) === 1, 422); $data=$request->validate(['value'=>['nullable','string','max:10000']]); return response()->json(SiteSetting::updateOrCreate(['key'=>$key],['value'=>$data['value'] ?? null])); }
